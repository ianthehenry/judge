...
(def- sequencey-forms '(
  fn match with with-dyns defn defn-
  varfn defmacro defmacro- defer edefer loop seq tabseq generate coro
  for each eachp eachk case cond do
  if when when-let when-with while with-syms with-vars
  if-let if-not if-with let try unless  forever upscope
  repeat forv compwhen compif ev/spawn ev/do-thread ev/with-deadline
  label prompt))
...
(defn prindent [form indentation &opt newline]
  ...
  (match (type+ form)
    (:ptuple (sequencey? (first form))) (do
      ...
      (eachp [i el] form
        (prindent el (if (= i 0) 0 indentation) (not= i (- (length form) 1)))
        )
      (prin ")"))
    (prinf "%q" form))
  (if newline (print)))
